...
class RemarkableInstaller:
    def __init__(self):
        ...
        # Local paths
        self.config_file = os.path.join(self.local_script_dir, "remarkable_scripts_config.json")
    ...
    def load_config(self):
            ...
            try:
                ...
                messagebox.showerror(
                    "Configuratie Fout",
                    f"Fout in JSON configuratie: {str(e)}"
                )
                self.scripts_config = {}
    ...
    def save_config(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Opslaan Fout",
                f"Kon configuratie niet opslaan: {str(e)}"
            )
    def connect_to_remarkable(self):
            ...
            try:
                ...
                try:
                    ssh.connect(
                        self.settings.remarkable_usb_ip,
                ...
                messagebox.showerror(
                    "Verbinding Fout",
                    f"Kon geen verbinding maken met reMarkable: {str(e)}"
                )
                return None
    def setup_gui(self):
        self.root = tk.Tk()
        ...
        # Install button
        install_btn = ttk.Button(main_frame, text="Installeer Scripts", 
                               command=self.start_installation)
        install_btn.grid(row=2, column=0, pady=5)

        # Reload config button
        reload_btn = ttk.Button(main_frame, text="Herlaad Configuratie", 
    ...
    def create_script_list(self, parent):
        ...
        self.list_frame = ttk.LabelFrame(parent, text="Te installeren scripts", padding="5")
    ...
    def update_script_list(self):
        ...
        # Add scripts
        for i, (script_name, config) in enumerate(sorted(
            self.scripts_config.items(), 
            key=lambda x: x[1]['order']
        )):
            ttk.Label(self.list_frame, text=str(config['order'])).grid(
                row=i+1, column=0)
            ttk.Label(self.list_frame, text=script_name).grid(
                row=i+1, column=1)
    ...
    def connect_ssh(self) -> paramiko.SSHClient:
        try:
            ...
            messagebox.showerror("Verbindingsfout", 
                               f"Kan niet verbinden met reMarkable: {str(e)}")
            raise
    ...
    def transfer_file(self, sftp, local_path: str, remote_path: str):
        try:
            ...
            sftp.put(
                os.path.join(self.local_script_dir, local_path),
                remote_path
            )
        except Exception as e:
            messagebox.showerror("Overzetfout", 
                               f"Fout bij overzetten {local_path}: {str(e)}")
            raise
    ...
    def execute_remote_script(self, ssh: paramiko.SSHClient, remote_path: str):
        try:
            ...
            if exit_status != 0:
                ...
                raise Exception(f"Script fout (exit code {exit_status}): {error_message}")
                
        except Exception as e:
            messagebox.showerror("Uitvoeringsfout", 
                               f"Fout bij uitvoeren {remote_path}: {str(e)}")
            raise
